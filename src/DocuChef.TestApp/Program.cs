...
* 슬라이드 1: #foreach: Categories, max: 1
  - ${Categories[0].Name}
* 슬라이드 2: #foreach: Categories_Products, max: 2
  - ${Categories_Products[0].Name}
  - ${Categories_Products[1].Name}
```
...
### 슬라이드 2: Food의 제품들 (Categories_Products, offset: 0)
- **바인딩 출력**: 
  - `${Categories_Products[0].Name}` → "Bibimbap"
  - `${Categories_Products[1].Name}` → "Bulgogi"

### 슬라이드 3: 카테고리 "Drinks" (Categories, offset: 1)
- **바인딩 출력**: 
  - `${Categories[0].Name}` → "Drinks"

### 슬라이드 4: Drinks의 제품들 (Categories_Products, offset: 0)
- **바인딩 출력**: 
  - `${Categories_Products[0].Name}` → "Americano"
  - `${Categories_Products[1].Name}` → "Green Tea"

### 슬라이드 5: Drinks의 제품들(추가) (Categories_Products, offset: 2)
- **바인딩 출력**: 
  - `${Categories_Products[0].Name}` → "Milk"
  - `${Categories_Products[1].Name}` → "" (비어있음 - 세 번째 항목만 존재)

### 슬라이드 6: 카테고리 "Desserts" (Categories, offset: 2)
- **바인딩 출력**: 
  - `${Categories[0].Name}` → "Desserts"

### 슬라이드 7: Desserts의 제품들 (Categories_Products, offset: 0)
- **바인딩 출력**: 
  - `${Categories_Products[0].Name}` → "Tiramisu"
  - `${Categories_Products[1].Name}` → "Chocolate Cake"

### 슬라이드 8: Desserts의 제품들(추가) (Categories_Products, offset: 2)
- **바인딩 출력**: 
  - `${Categories_Products[0].Name}` → "Fruit Salad"
  - `${Categories_Products[1].Name}` → "Macaron"

### 슬라이드 9: Desserts의 제품들(추가) (Categories_Products, offset: 4)
- **바인딩 출력**: 
  - `${Categories_Products[0].Name}` → "Pudding"
  - `${Categories_Products[1].Name}` → "" (비어있음 - 다섯 번째 항목만 존재)
...
2. **바인딩 표현식 해석**:
   ...
   - `${Categories_Products[0].Name}`, `${Categories_Products[1].Name}`는 현재 제품 슬라이드에서 표시될 제품들의 이름을 가져옵니다.

3. **그룹화 처리**:
   - `max: 2` 설정으로 인해 각 제품 슬라이드는 최대 2개의 제품만 표시합니다.
   - 두 개 이상의 제품이 있는 경우(Drinks, Desserts) 추가 슬라이드가 생성됩니다.
   - 제품 수가 홀수인 경우(Drinks: 3개, Desserts: 5개) 마지막 슬라이드에서 두 번째 바인딩(`${Categories_Products[1].Name}`)은 빈 값이 됩니다.

4. **컨텍스트 관리**:
   - 각 슬라이드는 자신의 특정 컨텍스트(카테고리 또는 제품 그룹)를 가지고 있습니다.
   - 중첩된 컬렉션(Categories_Products)에서는 부모 컨텍스트(현재 카테고리)도 유지되어 슬라이드 간의 관계가 보존됩니다.
 */
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                var pptChef = new PPTChef(templatePath, data);

                // 1. Run template analysis and display results
                Console.WriteLine("\n1. Analyzing template...");
                var analysisResult = pptChef.AnalyzeTemplate();

                Console.WriteLine($"   - Total {analysisResult.TotalSlides} slides found");
                Console.WriteLine($"   - Regular slides: {analysisResult.RegularSlides}");
                Console.WriteLine($"   - Foreach directives: {analysisResult.ForeachSlides}");
                Console.WriteLine($"   - If directives: {analysisResult.IfSlides}");

                // 2. Create presentation plan and display results
                Console.WriteLine("\n2. Creating presentation plan...");
                var plan = pptChef.CreatePlan();
                var planSummary = plan.GetSummary();

                Console.WriteLine($"   - Plan summary: {planSummary}");
                Console.WriteLine($"   - Slides to include: {planSummary.TotalIncludedSlides}");
                Console.WriteLine($"   - Slides to skip: {planSummary.SkippedSlides}");

                // Print detailed slide plan information
                Console.WriteLine("\n3. Slide plan details:");
                int slideIndex = 1;
                foreach (var slide in plan.IncludedSlides)
                {
                    string operation = slide.Operation == SlideOperation.Keep ? "Original" : "Clone";
                    string contextDesc = slide.HasContext ? slide.Context.GetContextDescription() : "Global data";

                    Console.WriteLine($"   - Slide {slideIndex++}: {operation}, {contextDesc}");
                }

                // 3. Generate presentation
                Console.WriteLine("\n4. Generating presentation...");
                pptChef.GeneratePresentation(outputPath, plan);
