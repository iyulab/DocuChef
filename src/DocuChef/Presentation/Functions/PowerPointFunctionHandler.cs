...
{
    private static readonly Regex ImagePlaceholderRegex = new(@"__PPT_IMAGE_([a-f0-9]{32})__([^_]+)(?:__(\d+)__(\d+)__(True|False))?__", RegexOptions.Compiled);    /// <summary>
    /// Processes all PPT function placeholders in a presentation
    /// </summary>
    ...
    private static void ProcessSlideImagePlaceholders(SlidePart slidePart, PPTFunctions pptFunctions)
    {
        ...
        foreach (var cacheItem in imageCache)
        {
            Logger.Debug($"ProcessSlideImagePlaceholders: Cache item key='{cacheItem.Key}', value length={cacheItem.Value.Length}");
        }
    ...
    private static void ProcessImagePlaceholder(SlidePart slidePart, DocumentFormat.OpenXml.Drawing.Text textElement, Match match, Dictionary<string, byte[]> imageCache)
    {
        try
        {
            ...
            if (!imageCache.TryGetValue(placeholder, out var imageData))
            {
            ...
            ImageHandler.ProcessImageData(shape, imageData, $"{propertyPath}.jpg");
