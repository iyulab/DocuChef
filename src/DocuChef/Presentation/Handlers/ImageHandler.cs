...
internal static class ImageHandler
{
    ...
    /// <summary>
    /// Processes an image using byte array data
    /// </summary>
    public static void ProcessImageData(P.Shape shape, byte[] imageData, string fileName = "image.jpg")
    {
        if (shape == null)
        {
            Logger.Error("Shape is null");
        ...
        try
        {
            ...
            {
                Logger.Debug($"Failed to delete temporary image file: {ex.Message}");
            }
        }
        catch (Exception ex)
        {
