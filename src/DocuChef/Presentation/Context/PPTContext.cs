...
public class PPTContext
{
    ...
    /// 현재 처리 단계
    /// </summary>
    public ProcessingPhase CurrentPhase { get; set; } = ProcessingPhase.Initialization;

    public PPTContext(PresentationDocument templateDocument, PowerPointOptions options)
    {
    ...
    {
        // 기존 이미지 캐시 보존
        var existingImageCache = Functions?.GetAllImageCache() ?? new Dictionary<string, byte[]>();
