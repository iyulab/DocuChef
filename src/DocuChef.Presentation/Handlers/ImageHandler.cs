...
internal static class ImageHandler
{
    ...
    private static readonly Dictionary<string, string> _cachedImages = new Dictionary<string, string>();
    ...
    /// </summary>
    public static void Process(P.Shape shape, string path, int width, int height, bool preserveAspectRatio)
    {
        if (shape == null)
        {
        ...
        try
        {
            ...
            var slidePart = shape.GetSlidePart();
            if (slidePart == null)
            {
    ...
    {
        return ClosedXML.Report.XLCustom.Functions.ImageHelper.GetImageFromPathOrUrl(path);
    }
    ...
    public static void Cleanup()
    {
        lock (_cacheLock)
        {
            foreach (var cachedPath in _cachedImages.Values)
            {
                try
                {
                ...
                {
                    Logger.Debug($"Error cleaning up cached image: {ex.Message}");
                }
            }
            _cachedImages.Clear();
        }
