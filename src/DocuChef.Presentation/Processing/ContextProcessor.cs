...
internal class ContextProcessor
{
        ...
        int maxItemsPerSlide = 0)
    {
        ...
        // Split collection name to get child property name
        string[] segments = nestedCollection.Split('_');
        ...
        {
            return GenerateGroupedNestedContexts(nestedCollection, parentContext, maxItemsPerSlide);
        }
        ...
        int maxItemsPerSlide)
    {
        ...
        // Get child items from parent context
        var childItems = parentContext.CurrentItem.GetCollection(childPropertyName);
        if (childItems == null)
            return result;

        // Count total items
        int totalItems = childItems.Count();
        if (totalItems == 0)
            return result;
        ...
        for (int groupIndex = 0; groupIndex < groupCount; groupIndex++)
        {
            ...
            Logger.Debug($"Created group context: offset={startIndex}, items={groupItems.Count}");
        }
